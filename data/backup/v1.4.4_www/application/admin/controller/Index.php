...
class Index extends Base
{
    public function index()
    {
        ...
        /*小程序开关*/
        $web_minipro_switch = tpCache('web.web_minipro_switch');
        $this->admin_lang != $this->main_lang && $web_minipro_switch = -1;
            ...
            if ($r !== false) {
                ...
                switch ($table) {
                    ...
                    case 'minipro_page':
                    {
                        if ('is_home' == $field) {
                            $data['refresh'] = 1;
                            Db::name('minipro_page')->where([
                                $id_name    => ['NEQ', $id_value],
                                'lang'      => $this->admin_lang,
                            ])->update(['is_home'    => 0, 'update_time'   => getTime()]);
                        }
                        break;
                    }
            ...
            switch ($inc_type) {
                ...
                case 'web':
                {
                    ...
                    if (in_array($name, ['web_users_switch'])) {
                        ...
                        $data['is_syn'] = $is_syn;
                        /*--end*/
                    } else if ($name = 'web_language_switch') { // 重新生成sitemap.xml
                        /* 生成sitemap */
                        sitemap_all();
                    }
                    break;
